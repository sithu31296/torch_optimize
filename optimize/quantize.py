...
def main(args):
    ...
    save_model = save_dir / f"{args.model}_optimized.pt"
    ...
    optimized_model = quantize_model(model, args.method, args.backend) 
    ...
    if args.lite_save:
        optimized_model._save_for_lite_interpreter(save_dir / f"{args.model}_optimized_lite.ptl")
    ...
    table = [
        [f"Original {args.model} (FP32)", f"{model_size:.2f}", f"{model_time:.2f}", f"{model_accuracy:.2f}"],
        [f"Quantized {args.model} (INT8)", f"{quantized_model_size:.2f}", f"{quantized_model_time:.2f}", f"{quantized_model_accuracy:.2f}"],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--method', type=str, default='static', help='quantization method `dynamic`, `static`, `qat` (for CNN models, use static or qat, for Transformer and MLP models, use dynamic)')
    parser.add_argument('--backend', type=str, default='qnnpack', help='backend engine (`qnnpack` for ARM, `fbgemm` for x86)')
    parser.add_argument('--mobile-opt', type=bool, default=False, help='use mobile optimizer')
    parser.add_argument('--lite-save', type=bool, default=False, help='save for using with lite interpreter')
